...
func TestBodyWriteDryRunPrintsDiffWithoutUpdating(t *testing.T) {
	...
	out := stdout.String()
	for _, want := range []string{"<!-- ai-summary:start -->", "line | op | content", "   1 |  - | old summary", "   1 |  + | new summary", "<!-- ai-summary:end -->"} {
...
func TestCommentUpsertDryRunDoesNotCreateMissingComment(t *testing.T) {
	...
	out := stdout.String()
	for _, want := range []string{"<!-- ai-review:start -->", "line | op | content", "   1 |  + | review", "<!-- ai-review:end -->"} {
		if !strings.Contains(out, want) {
